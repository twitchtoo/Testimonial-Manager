...
$page_query = $db->Execute("select testimonials_id, tm_rating, testimonials_image, testimonials_title, testimonials_html_text, date_added  from " . TABLE_TESTIMONIALS_MANAGER . " where status = 1  and tm_make_public = 'yes' and language_id = " . (int)$_SESSION['languages_id'] . " order by rand(), testimonials_title limit " . MAX_DISPLAY_TESTIMONIALS_MANAGER_TITLES ."");
if ($page_query->RecordCount()>0) {
	...
	$page_query_list[$rows]['rating'] = $page_query->fields['tm_rating'];	
